...
| :--- | :--- | :--- |
| `this.canvas` | HTML \<canvas\> | DOM canvas element that will hold the final image. |
...
{`new class {
    ...
    templateInstructions() {
        ...
        // highlight-start
        this.canvas.addLayer('multiply').draw(function() {
            ...
        });
        // highlight-end
...
```javascript
.addLayer(type = 'normal', effect = null)
```

| # | Name | Required | Type | Default | Description |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | `type`  | ❌ | String | `'normal'` | The layer type, or the effect. `'normal'` \| `'alpha'` \| or a knowned CSS effect |
| 2 | `effect`  | ❌ | String | *`null`* | An optional second effect to apply on the layer. |
...
| `'alpha'` | The current layer opacity is determined by the opacity of the following drawn layer. |
| `'luminance'` | The current layer opacity is determined by the luminance of the following drawn layer. |
| **String** | Any knowned [globalCompositeOperation properties](https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D/globalCompositeOperation). Possible values are `'screen'`, `'overlay'`, `'darken'`, `'lighten'`, `'color-dodge'`, `'color-burn'`, `'hard-light'`, `'soft-light'`, `'difference'`, `'exclusion'`, `'hue'`, `'saturation'`, `'color'`, `'luminosity'`|


:::note

You can cumulate 'alpha' and any other effect by providing the two string separated. e.g.

```javascript
this.canvas.addLayer('alpha', 'multiply').draw(function() {
    ...
});
```
...
{`new class {
    constructor() { 
        ...
        // Download logic
        // highlight-next-line
        this.container.querySelector('.myButton').addEventListener('click', () => this.canvas.download());
    }
    templateInstructions() { ... }
}`}
</CodeBlock>
